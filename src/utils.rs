...
mod tests {
    ...
    #[test]
    fn get_editor_returns_editor_from_env_var() {
        let original_editor = var("EDITOR");

        set_var("EDITOR", "/usr/bin/fake_editor");
        let editor = get_editor().unwrap();
        assert_eq!(editor, "/usr/bin/fake_editor");

        // restore environment variable state
        match original_editor {
            Ok(x) => set_var("EDITOR", x),
            Err(_) => remove_var("EDITOR")
        }
    }
    
    #[test]
    fn get_editor_returns_vim_or_errors_if_editor_env_var_empty() {
        remove_var("EDITOR");
        let editor = get_editor();
        match editor {
            Ok(x) => {
                assert!(x.len() > 0);
                assert!(x.contains("vim"));
            },
            Err(y) => assert_eq!(y.to_string(), "failed to determine editor")
        }
    }
}
